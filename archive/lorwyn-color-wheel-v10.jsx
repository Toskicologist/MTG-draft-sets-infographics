 ...
 * v10 React - Added Scryfall integration
 *
 * INSTRUCTIONS FOR FUTURE EDITORS:
 * When making changes, please:
 * 1. Increment the version number (v10 -> v11, etc.)
 * 2. Update the version in the page subtitle
 * 3. Add an entry to the changelog below with your changes
 * 4. Include the date of your changes
 *
 * CHANGELOG:
...
const InfoPanel = ({ pair }) => {
  if (!pair) return null;

  const labelData = labels[pair];
  const { bg, fg } = getLabelColors(pair[0], pair[1]);

  // Construct Scryfall search URL for this color pair in Lorwyn Eclipsed
  const scryfallUrl = `https://scryfall.com/search?q=set%3Aecl+color%3D${pair}`;
    ...
    <div style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: '16px' }}>
        ...
        <p style={{ fontSize: '0.95rem', color: '#6B7280', margin: 0 }}>
          Draft Archetypes Color Wheel (v10 Interactive)
        </p>
      </div>
      ...
      >
        ...
        {colorOrder.map((color, i) => {
          const isHighlighted = activeColors.includes(color);
          const isDimmed = activeColors.length > 0 && !isHighlighted;
          return (
            <ColorNode
              key={color}
              color={color}
        ...
        {labelPositions.map((group, i) => {
          ...
          return (
            <LabelBox
              ...
              isHighlighted={isHighlighted}
              isDimmed={isDimmed}
              onHover={() => setHoveredPair(pairCode)}
